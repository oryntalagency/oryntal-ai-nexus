...
import require$$6 from "dns";
import require$$8 from "os";
...
function requireCookies() {
  ...
  class Cookies {
     ...
     */
    set(cookieStr, url2) {
      const urlparts = urllib.parse(url2 || "");
...
function requireObjects() {
  if (hasRequiredObjects) return objects.exports;
...
function requireFetch() {
  ...
  const { isProtoKey } = requireObjects();
  ...
  function nmfetch(url2, options) {
    ...
    req.on("response", (res) => {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        ...
        let redirectUrl;
        try {
          redirectUrl = urllib.resolve(url2, res.headers.location);
        } catch (_err) {
...
function requireShared() {
  ...
  (function(module) {
    ...
    const errors2 = requireErrors();
    const objects2 = requireObjects();
    const dns = require$$6;
    const net = require$$7;
    const os = require$$8;
    const isProtoKey = module.exports.isProtoKey = objects2.isProtoKey;
    module.exports.copyOwnKeys = objects2.copyOwnKeys;
    ...
    module.exports.parseConnectionUrl = (str) => {
      str = str || "";
      const options = {};
      const url2 = urllib.parse(str, true);
    ...
    function resolveContentValue(data, key, options, callback) {
      ...
      if (typeof content === "object") {
        if (typeof content.pipe === "function") {
          ...
          return resolveStream(nmfetch(url2, { headers: content.httpHeaders, tls: content.tls }), callback);
...
function requireMimeNode() {
  ...
  class MimeNode {
      ...
      this._headers.forEach((header) => {
        ...
        switch (header.key) {
          ...
          case "Content-Type":
            ...
            value = mimeFuncs2.buildHeaderValue(structured);
            if (this.filename) {
              param = /[\x00-\x1f\x7f]/.test(this.filename) ? mimeFuncs2.encodeWord(this.filename, this._getTextEncoding(this.filename), 52) : this._encodeWords(this.filename);
    ...
    _normalizeLocalPart(user) {
      ...
      return mimeFuncs2.quoteString(user);
    }
    /**
    ...
    _encodeAddressName(name2) {
      if (!/^[\w ]*$/.test(name2)) {
        if (/^[\x20-\x7e]*$/.test(name2)) {
          return mimeFuncs2.quoteString(name2);
        } else {
    ...
    _encodeHeaderText(value) {
      return /[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(value) ? mimeFuncs2.encodeWord(value, this._getTextEncoding(value), 52) : (
...
function requireMailComposer() {
  ...
  const mimeFuncs2 = requireMimeFuncs();
  const { parseDataURI, copyOwnKeys } = requireShared();
  class MailComposer {
    constructor(mail) {
      this.mail = mail || {};
      this.message = false;
...
function requireDkim() {
  ...
  const path = require$$0$5;
  const crypto = require$$1$1;
  const { copyOwnKeys } = requireObjects();
...
function requireMailMessage() {
  ...
  class MailMessage {
    constructor(mailer2, data) {
      ...
      this.message = null;
      data = data || {};
      const options = mailer2.options || {};
      const defaults = mailer2._defaults || {};
        ...
        value: [].concat(listData[key] || []).map((value) => ({
          ...
          value: [].concat(value || []).map((value2) => {
            ...
            if (value2 && value2.url) {
              ...
              const needsEncoding = !mimeFuncs2.isPlainText(comment) || /\x7f/.test(comment);
              if (key.toLowerCase().trim() === "id") {
                comment = needsEncoding ? mimeFuncs2.encodeWord(comment) : mimeFuncs2.quoteString(comment);
                return (value2.comment ? comment + " " : "") + this._formatListUrl(value2.url).replace(/^<[^:]+:\/{0,2}/, "<");
              }
              comment = needsEncoding ? mimeFuncs2.encodeWord(comment) : comment.replace(/[()\\]/g, "\\$&");
        ...
        this._processPlugins("stream", mail, (err2) => {
          ...
          if (mail.data.dkim || this.dkim) {
            mail.message.processFunc((input) => {
              const dkim2 = mail.data.dkim ? new DKIM(mail.data.dkim) : this.dkim;
              this.logger.debug(
                {
                  tnx: "DKIM",
  ...
  class SMTPConnection extends EventEmitter {
    ...
    _createSendStream(callback) {
      ...
      dataStream2.once("end", () => {
        if (this._currentDataStream === dataStream2) {
          this._currentDataStream = false;
        }
        this.logger.info(
          {
...
function requireXoauth2() {
  ...
  class XOAuth2 extends Stream {
    ...
    postRequest(url2, payload, params, callback) {
      ...
      const req = nmfetch(url2, fetchOptions);
      req.on("readable", () => {
        let chunk;
...
const TurboSMTP = { "description": "TurboSMTP", "host": "pro.turbo-smtp.com", "port": 465, "secure": true };
...
const require$$0 = {
  ...
  Tipimail,
  TurboSMTP,
  "TurboSMTP-EU": { "description": "TurboSMTP (EU region)", "host": "pro.eu.turbo-smtp.com", "port": 465, "secure": true },
  Tutanota,
  Yahoo,
  Yandex,
  Zimbra,
...
function requireSesTransport() {
  ...
  class SESTransport extends EventEmitter {
    ...
    send(mail, callback) {
      ...
      if (fromHeader) {
        const mimeNode2 = new MimeNode("text/plain");
